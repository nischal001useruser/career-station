...
export function InputResultPage() {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        setIsLoading(true)
        const response = await fetch(`/api/exams/${selectedExamId}/questions`)
        const data = await response.json()
...
export function PastResultsPage() {
  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      ...
      try {
        const response = await fetch(`/api/results/leaderboard/${selectedExamId}`)
        const data = await response.json()
...
export function StudentsPage() {
  ...
  const loadStudentAnalytics = async (student) => {
    ...
    try {
      const response = await fetch(`/api/results/analytics/student/${student.id}`)
      const data = await response.json()
